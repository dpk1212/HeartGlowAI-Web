    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (resultCard) {
        resultCard.addEventListener('click', function(e) {
          // Check if the click was on the close button (the ::after pseudo-element)
          // We can detect this by checking the position of the click relative to the element
...
Maintain the core message and emotional intent while applying these changes.`
    ...
    // Add event listener for the close button function
    function setupResultCardCloseButton() {
      const resultCardLocal = document.getElementById('result');
      if (resultCardLocal) {
        resultCardLocal.addEventListener('click', function(e) {
          // Check if the click was on the close button (the ::after pseudo-element)
          // We can detect this by checking the position of the click relative to the element
          const rect = resultCardLocal.getBoundingClientRect();
          const isCloseButtonClick = (
            e.clientX > rect.right - 40 &&
            e.clientX < rect.right &&
            e.clientY > rect.top &&
          ...
          if (isCloseButtonClick) {
            resultCardLocal.style.display = 'none';
          }
        });
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
      // Setup the close button for the result card
      setupResultCardCloseButton();
