...
const ChatMessageList: React.FC<{ messages: Message[]; isLoading: boolean }> = ({ messages, isLoading }) => {
  ...
  }, [messages, isLoading]); // Rerun on messages or loading state change
    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
      {messages.map((msg) => (
        ...
        >
          ...
          <div
            className={cn(
              "max-w-xs md:max-w-md lg:max-w-lg rounded-lg px-4 py-2 text-sm shadow-sm", // Added shadow-sm
              msg.role === 'user'
                ? 'bg-primary text-primary-foreground rounded-br-none'
                : 'bg-card border border-border/50 rounded-bl-none' // Use card bg for assistant
            )}
            // Add dangerouslySetInnerHTML if content can be HTML, otherwise keep as is
          ...
          {msg.role === 'user' && (
             <Avatar className="h-8 w-8 flex-shrink-0">
               {/* TODO: Replace with actual user image/initials */}
      ...
      {/* Add a small spacer at the bottom to prevent input overlap */}
       <div className="h-2" />
    </div>
  );
};
