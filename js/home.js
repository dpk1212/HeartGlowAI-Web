 ...
 * Ensures both the Save and Cancel buttons in the connection modal work properly
 */
function fixConnectionModalButtons() {
  console.log('Fixing connection modal buttons in home.js...');
  
  // Get modal and buttons
  const modal = document.getElementById('connection-modal');
  const form = document.getElementById('connection-form');
  const saveButton = modal?.querySelector('.btn-primary, button[type="submit"]');
  const cancelButton = modal?.querySelector('.btn-secondary, #cancel-connection');
  ...
  // Fix form submission
  if (form) {
    // Clone form to remove existing handlers
    ...
    // Need to reattach event handlers to buttons since we replaced the form
  ...
  if (cancelButton) {
    ...
    newCancelButton.style.display = 'inline-block';
  ...
  console.log('Connection modal buttons fixed in home.js');
}

// Add initialization
document.addEventListener('DOMContentLoaded', function() {
  console.log('DOMContentLoaded - fixing connection modal buttons in home.js');
  
  // Fix the buttons on page load
  setTimeout(fixConnectionModalButtons, 500);
  
  // Override openConnectionModal to fix buttons when modal is opened
  const originalOpenConnectionModal = window.openConnectionModal || openConnectionModal;
  window.openConnectionModal = function() {
    console.log('openConnectionModal called in home.js - applying original function');
