...
const DashboardTour: React.FC<DashboardTourProps> = ({ isActive, onComplete }) => {
  ...
  // --- Positioning Effect (Desktop Only) ---
  useEffect(() => {
    // Only run positioning logic if active, not mobile, and have a target selector
    if (isActive && !isMobile && currentStepData?.targetSelector) {
      const timer = setTimeout(() => {
        const targetElement = document.querySelector<HTMLElement>(currentStepData.targetSelector);
        
        if (targetElement) {
          // Bring element into view before calculating rect
          targetElement.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });

          // Add another short delay after scrolling to allow layout to settle
          ...
          }, 100); // Delay after scroll

          return () => clearTimeout(posTimer);
        } else {
          console.warn(`DashboardTour: Target element not found: ${currentStepData.targetSelector}`);
          setTargetRect(null);
          // Reset to center if target not found (even on desktop)
          setTooltipPosition({ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' });
          setArrowPosition(null);
        }
      }, 150); // Initial delay for element query

      return () => clearTimeout(timer);
    } else if (isMobile) {
        // Reset positions for mobile modal view
        setTargetRect(null);
        setTooltipPosition({ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' });
        setArrowPosition(null);
    }
  }, [isActive, currentStepIndex, currentStepData, isMobile]); // Add isMobile dependency
  ...
  // Calculate overlay style using clip-path for cutout effect
   ...
   const overlayStyle: React.CSSProperties = targetRect ? {
       ...
       // Apply clip-path only if NOT mobile and targetRect exists
       clipPath: !isMobile ? `polygon(
         0% 0%, 0% 100%, 
         ...
         ${targetRect.left - overlayPadding}px 100%,
         100% 100%, 100% 0%
       )` : 'none', // No clip-path cutout on mobile
   } : {
       // Default: cover everything if no target or mobile
       clipPath: 'none',
   };

  const tooltipStyle: React.CSSProperties = {
    position: 'absolute',
    // Use calculated position for desktop, centered for mobile
    top: isMobile ? '50%' : tooltipPosition.top,
    left: isMobile ? '50%' : tooltipPosition.left,
    transform: isMobile ? 'translate(-50%, -50%)' : tooltipPosition.transform,
    zIndex: 1150,
  };
  ...
  return (
    ...
    >
      ...
      <motion.div 
          ...
          style={tooltipStyle} 
      >
          {/* Arrow Element - Only render on desktop */}
          {!isMobile && arrowPosition && <div style={arrowFinalStyle}></div>}
