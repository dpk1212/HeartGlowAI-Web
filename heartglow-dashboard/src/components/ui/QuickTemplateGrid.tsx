...
const TemplateCard: React.FC<{ template: Template; index: number }> = ({ template, index }) => {
  return (
    <Link 
      href={getRouteWithBasePath(`/create?intent=${template.intent}&tone=${template.tone}`)}
      className="bg-white dark:bg-heartglow-deepgray p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-800 flex flex-col group transform hover:-translate-y-1 animate-fadeIn"
    ...
    >
      ...
      <h3 className="text-lg font-bold mb-2 text-heartglow-charcoal dark:text-heartglow-offwhite group-hover:text-heartglow-pink dark:group-hover:text-heartglow-pink transition-colors duration-300">
        {template.title}
      </h3>
